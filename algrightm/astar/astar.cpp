...
bool AStar::canReach(const Vec2 &current_pos, const Vec2 &target_pos, bool corner)
{
	if (target_pos.x >= 0 && target_pos.x < width_ && target_pos.y >= 0 && target_pos.y < height_)
	{
		...
		{ 
			return query_(target_pos);                     /// 判断是否为障碍物
		}
		else if (corner)                                   /// 是否考虑斜角移动
   		{   
			return (canReach(Vec2(current_pos.x + target_pos.x - current_pos.x, current_pos.y))
					|| canReach(Vec2(current_pos.x, current_pos.y + target_pos.y - current_pos.y))) &&  query_(target_pos);
			/*
			return (canReach(Vec2(current_pos.x + target_pos.x - current_pos.x, current_pos.y))
					&& canReach(Vec2(current_pos.x, current_pos.y + target_pos.y - current_pos.y)) && query_(target_pos));
...
void AStar::findCanReachPos(const Vec2 &current_pos, bool corner, std::vector<Vec2> &can_reach)
{
	...
	int row_index = current_pos.y - 1;          /// 周围方块的行索引起始
	const int max_row = current_pos.y + 1;      /// 周围方块索引范围
...
bool cmp(const AStar::Node* a,const AStar::Node* b)  ///比较函数
{
	return a->f() > b->f();
}
...
#if MSVC08
	std::push_heap(open_list_.begin(), open_list_.end(), cmp);
#endif
	...
	else
	{
		...
		while (!open_list_.empty())
		{
...
#if MSVC08
			std::pop_heap(open_list_.begin(), open_list_.end(), cmp);
#endif
			...
			while (index < size)
			{
				...
				if (hasNoodeInOpenList(nearby_nodes[index], new_node))  /// 已在OpenList表中
				...
				else
				{
					...
					if (nearby_nodes[index] == param.end)             /// 终点
					{
						while (new_node->parent)                      /// 回溯
						{
