/*================================================================================================================================
+
+                                  KeyWord Recognition using dynamic time warping
+
+      算法思路：建立keyword的MFCC特征序列作为模板，使用dynamic time warping算法计算最短规整距离，并与阈值比较
+       
+           1）提取MFCC
+           2）DTW模板匹配
+          
+   NOTE：调用winmm.lib实现录音
+
+
+=================================================================================================================================*/
...
double simpleVAD(char* waveData,int frameSize)
{
	...
	for(i=0;i<frameSize;i++)
	{
		if(nBytesPerSample==2) 
			temp = (double)*((short*)waveData+i);
		else if(nBytesPerSample ==1) 
			temp = (double)*((bool*)waveData+i);
		else if(nBytesPerSample == 4)
			temp = (double)*((int*)waveData+i);
		if(temp>0)
			vad += temp;
		else
			vad -= temp;
...
double dtw(PTRFEAT pf1, PTRFEAT pf2)
{
	...
	if(pf1->vl != pf2->vl)
	{ 
		fprintf(stderr,"feature vector size not equal!\n"); 
		return -1; 
	}
	n = pf1->vl;
	len = pf1->vs*pf2->vs;
...
int main() 
{
	...
	FILE *fpvad = fopen("vad.log","w+");
	FILE *fplbl = fopen("vad.lbl","w+");
	FILE *fpres = fopen("cmd.log","w+");
	char str[512];
	SYSTEMTIME st;
	GetLocalTime(&st);
	sprintf(str,"==================================================\n"
    ...
    /// 读入模板库
	readFeatFromFile(&featleft, "model_left.mfcc");
	readFeatFromFile(&featforward, "model_forward.mfcc");
	readFeatFromFile(&featright, "model_right.mfcc");
	...
	while(waveHdrAll.dwBytesRecorded/nBytesPerSample<dataSizeAll-frameSize)
	{
		...
		if(waveHdrAll.dwBytesRecorded/nBytesPerSample>(i-1)*frameStep+frameSize)
		{
			...
			else if(sdStart == true && ((vad<thres && i-pi>fmin)||i-pi>fmax))
			{
				...
				sprintf(fn,"test_%03d_%s.mfcc", j, res);
	...
	SaveRecordtoFile("myTestAll.wav",&wf,&waveHdrAll,&mmtAll);
