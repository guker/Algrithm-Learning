...
int _weightwin(PTRFRAME frame,int win)
{
	...
	if(_isWInit == false)
	{
		...
		case 1:  // Hamming window
			for(i=0;i<n;i++)
			{ 
				*(_w+i) = (double) (0.54-0.46*(cos(PI2*(double)i/(n))));
			}
			break;
		case 2:  // Hanning window
			for(i=0;i<n;i++)
			{ 
				*(_w+i) = (double)(0.5-0.5*(cos(PI2*(double)i/(n-1)))); 
			}
			break;
		case 3:  // Blackman window
			for(i=0;i<n;i++)
			{ 
				*(_w+i) = (double)(0.42-0.5*(cos(PI2*(double(i)/(n-1))))+0.08*(cos(PI4*(double(i)/(n-1)))));
			}
			break;
		default:
			for(i=0;i<n;i++)
...
int _fft_init(int npts,int *_fftm)
{
	...
	for(i=2;i<=n16;i++)
	{ // use trigonometric formula to calculate the remain values
